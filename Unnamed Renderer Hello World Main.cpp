// Unnamed Renderer Hello World Main.cpp - Let's get some color swirling action.
// Mar 2020
// Chris M.
...
int __stdcall wWinMain(HINSTANCE, HINSTANCE, LPWSTR, int) {
	...
	struct RootConstants{
		float InitialRedCircleOriginX{ 1.0f }, InitialRedCircleOriginY{ 0.0f };
		float InitialGreenCircleOriginX{ 1.0f }, InitialGreenCircleOriginY{ 1.0f };
		float InitialBlueCircleOriginX{ 0.0f }, InitialBlueCircleOriginY{ 1.0f };
		float MainCircleOriginX{ 0.50f }, MainCircleOriginY{ 0.50f };
		float CircleRadii{ 1.0f };
		int CurrentFinalFrameIndex{ 0u };
	...
	for (__int64 i{ 0 }; i < FinalFrameCount; i++) {
		...
		RootConstants.CurrentFinalFrameIndex = static_cast<int>(i);
		RootConstants.GlobalTickInRadians = (static_cast<float>(i) / static_cast<float>(FinalFrameCount)) * 2.0f;
		RenderCommandLists[i]->GetInterface()->SetComputeRoot32BitConstants(1u, sizeof(RootConstants) / sizeof(float),
			&RootConstants, 0u);
	...
	// WindowClass, Window, and SwapChain.
	...
	while (true == true)
	{
		PeekMessageW(&MessageStruct, NULL, MessageFilterMin, MessageFilterMax, PM_REMOVE);

		if (MessageStruct.message == WM_RBUTTONDOWN)
		{
			// Exit the program.
			break;
		}
		else if (MessageStruct.message == WM_MBUTTONDOWN)
		{
			// Rendering Logic.
			for (__int64 i{ 0 }; i < FinalFrameCount; i++) {
		...
		else if (IsDialogMessageW(TheatreWindow.ReportWindowHandle(), &MessageStruct))
		{
	...
	// Test message.
	if (DEBUG_ENABLED) {
		MessageBox(NULL, L"Success!", L"Dev Message", NULL);
	}
	...
	// Cleanup (Perhaps tuck this into some container classes).
