...
namespace WinRTStuff {
	...
	public:
		...
		~WindowClass() {
			...
			Result = UnregisterClass(this->ClassName, GetModuleHandle(NULL));

			if (Result == 0) {
				FailBail(L"UnregisterClass failed.", L"WinRTStuff::WindowClass Error");
			}
		}
	...
	public:
		...
		RenderWindow(const wchar_t* WindowClassName, unsigned __int32 ViewPixelWidth, unsigned __int32 ViewPixelHeight,
			...
			this->WindowHandle = CreateWindowEx(NULL, WindowClassName, WindowTitle,
				WS_VISIBLE, 0, 0, TotalWidth, TotalHeight, NULL, NULL, GetModuleHandle(NULL), NULL);
		...
		~RenderWindow() {
			...
			Result = DestroyWindow(this->WindowHandle);

			if (Result == 0) {
				FailBail(L"DestroyWindow failed.", L"WinRTStuff::RenderWindow Error");
			}
